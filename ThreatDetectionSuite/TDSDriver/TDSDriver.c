...
void DispatchPendingEvents() {
    ...
    while (TRUE) {
        ...
        if (IsListEmpty(&g_EventQueueHead)) { KeReleaseSpinLock(&g_EventQueueLock, eventIrql); KeReleaseSpinLock(&g_IrpQueueLock, irpIrql); break; }
        ...
        if (IoSetCancelRoutine(Irp, NULL) == NULL) {
            ...
            KeReleaseSpinLock(&g_EventQueueLock, eventIrql); KeReleaseSpinLock(&g_IrpQueueLock, irpIrql); continue;
        }
        RemoveEntryList(&pIrp->ListEntry);
        PLIST_ENTRY eventEntry = RemoveHeadList(&g_EventQueueHead); g_EventQueueCount--;
        PEVENT_ITEM pEvent = CONTAINING_RECORD(eventEntry, EVENT_ITEM, ListEntry);
        KeReleaseSpinLock(&g_EventQueueLock, eventIrql); KeReleaseSpinLock(&g_IrpQueueLock, irpIrql);
        ...
        IoCompleteRequest(Irp, IO_NO_INCREMENT);
        ExFreePoolWithTag(pIrp, 'SDTe'); ExFreeFromNpagedLookasideList(&g_EventLookasideList, pEvent);
    }
}

void QueueTDSEvent(PEVENT_ITEM item) {
    KIRQL irql; KeAcquireSpinLock(&g_EventQueueLock, &irql);
    if (g_EventQueueCount >= EVENT_QUEUE_LIMIT) { KeReleaseSpinLock(&g_EventQueueLock, irql); ExFreeFromNpagedLookasideList(&g_EventLookasideList, item); return; }
    InsertTailList(&g_EventQueueHead, &item->ListEntry); g_EventQueueCount++;
...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath) {
    ...
    InitializeListHead(&g_EventQueueHead); KeInitializeSpinLock(&g_EventQueueLock);
